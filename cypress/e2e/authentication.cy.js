describe("Authentication", () => {
  beforeEach(() => {
    cy.visit("http://104.236.4.147:3000/auth/login");
  });
  it("should log in successfully", () => {
    cy.intercept("POST", "/auth/login").as("loginRequest");

    cy.visit("http://104.236.4.147:3000/login");
    ...
    cy.wait("@loginRequest").then((interception) => {
      if (interception.response.statusCode === 200) {
        cy.url().should("eq", "http://104.236.4.147:3000/dashboard");
      } else {
        cy.contains("Signup here").click();
        cy.url().should("eq", "http://104.236.4.147:3000/signup"); 
        ...
        cy.url().should("eq", "http://104.236.4.147:3000/login");
